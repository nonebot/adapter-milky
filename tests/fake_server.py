...
from wsproto.events import AcceptConnection, BytesMessage, CloseConnection, Ping, TextMessage
...
http_handlers: "Queue[Callable[[Request], Response]]" = Queue()
ws_handlers: "Queue[Callable[[dict], dict]]" = Queue()
...
def http_echo(request: Request) -> Response:
    ...
    try:
        handler = http_handlers.get(timeout=2)
        ...
        for event in ws.events():
            if isinstance(event, WSRequest):
                ...
                if event.data == "exit":
                    ...
                    _json = json.loads(event.data)
                    try:
                        handler = ws_handlers.get(timeout=2)
                        out_data += ws.send(TextMessage(data=json.dumps(handler(_json), ensure_ascii=False)))
                    except Empty:
